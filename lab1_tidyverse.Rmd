...
Add the information they put together here
...
```{r}
libraries_subset = select(libraries, library_name, city, total_bldg_sqft)
```
...
```{r}
libraries %>% select(library_name, starts_with("address"), door_count, everything())
```
...
```{r}
libraries %>%
  filter(city == "Denver", branch_code %in% c("BDY", "UNH", "CRK", "FIE"), door_count > 80000)
```
...
libraries_new = libraries %>%
  mutate(high_circulation = ifelse(circulation > median(circulation), TRUE, FALSE),
         favorite_food = "pizza",
         double_door_count = door_count * 2) %>%
  select(high_circulation, favorite_food, double_door_count, everything())
...
```{r}
libraries %>%
  summarize(median = median(circulation), mean = mean(circulation), variance = var(circulation))
```
...
libraries_new %>%
  ...
  summarize(mean_count = mean(door_count), mean_sqft = mean(total_bldg_sqft)) %>%
  ungroup() %>%
  knitr::kable()
...
```{r, eval = FALSE}
ggplot(libraries, aes(x = circulation)) +
  geom_histogram() +
  ggtitle("Circulation of books in Denver libraries")
```
...
```{r}
libraries %>%
  ggplot(aes(x = circulation, y = total_bldg_sqft)) +
...
libraries_new %>%
  filter(branch_code != "CEN") %>%
  ggplot(aes(x = door_count, y = total_bldg_sqft)) +
  geom_point(color = "orange") +
  geom_smooth(method = "lm", se = FALSE)  +
